...
void OnDeinit(const int reason)
  {
   ...
   for(int report_trade=0;report_trade<ArraySize(finalized_trades);report_trade++){if(finalized_trades[report_trade].entry.strategy_type=="TREND_CONTINUATION")tc_finalized++;else if(finalized_trades[report_trade].entry.strategy_type=="RANGE_MEAN_REVERSION")rmr_finalized++;else if(finalized_trades[report_trade].entry.strategy_type!="RANGE_BREAKOUT")cross_setup_contamination++;}
