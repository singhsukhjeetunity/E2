...
void E2EmitVerification(void)
  {
   ...
   g_backtest_summary.PlanVerify(plan);g_backtest_summary.ExecVerify(g_execution_verify);g_backtest_summary.RVerify(g_r_verify);g_backtest_summary.DayVerify(g_adxbb_recovery.DayVerification());g_backtest_summary.DayRecoveryDiag(g_adxbb_recovery.DayRecoveryDiagnostics());g_backtest_summary.RecoveryVerify(g_adxbb_recovery.Verification());g_backtest_summary.RecoveryDiag(g_adxbb_recovery.Diagnostics());g_backtest_summary.RecoveryDiagFields(g_adxbb_recovery.Diagnostics());g_backtest_summary.RecoveryDiagComparisons(g_adxbb_recovery.Diagnostics());
...
void OnTick()
  {
   ...
   for(int i=0;i<ArraySize(candidates);i++)
     {
      E2OrderRequest request;if(!g_adxbb_planner.Build(candidates[i],request))continue;g_trade_requests++;
      g_execution_verify.attempts++;E2ExecutionResult result;
      if(!g_order_executor.Execute(request,"E2ADXBB|"+StringSubstr(candidates[i].candidate_id,0,20),result)){g_execution_verify.failures++;continue;}
