...
string E2ActiveStrategyName(void)
  {
   if(g_configuration.strategy_mode==E2_STRATEGY_XAU_SESSION_FADE)return("XAU_SESSION_FADE");
   return("LONDON_RANGE_BREAKOUT");
  }
...
bool E2SymbolAllowedForStrategy(void)
  {
   ...
   if(g_configuration.strategy_mode==E2_STRATEGY_LONDON_BREAKOUT)
...
void E2EmitVerification(void)
  {
   ...
   E2PlanVerification plan;
   if(g_configuration.strategy_mode==E2_STRATEGY_XAU_SESSION_FADE)plan=g_xau_planner.Verification();
   else plan=g_london_planner.Verification();
   ...
   E2SignalVerification signal;
   if(g_configuration.strategy_mode==E2_STRATEGY_XAU_SESSION_FADE)signal=g_xau_engine.SignalVerification();
   else signal=g_london_engine.SignalVerification();
   g_backtest_summary.SignalVerify(g_configuration.strategy_mode==E2_STRATEGY_XAU_SESSION_FADE?"XAU_SF_SIGNAL_VERIFY":"LRB_SIGNAL_VERIFY",signal);
...
void OnTick()
  {
   ...
   E2Candidate candidates[];if(g_configuration.strategy_mode==E2_STRATEGY_XAU_SESSION_FADE){if(!g_xau_engine.Evaluate(candidates,g_position_recovery))return;}else if(!g_london_engine.Evaluate(candidates,g_position_recovery))return;
   E2SignalVerification verification;
   if(g_configuration.strategy_mode==E2_STRATEGY_XAU_SESSION_FADE)verification=g_xau_engine.SignalVerification();
   else verification=g_london_engine.SignalVerification();
   g_strategy_candidates=(int)verification.total_candidates;
   for(int i=0;i<ArraySize(candidates);i++)
     {
      g_trade_reporter.BeginCandidate(candidates[i]);E2OrderRequest request;E2PlanningAudit audit;bool planned=(g_configuration.strategy_mode==E2_STRATEGY_XAU_SESSION_FADE?g_xau_planner.Build(candidates[i],request,audit):g_london_planner.Build(candidates[i],request,audit));g_trade_reporter.RecordPlanning(candidates[i].candidate_id,audit);if(!planned)continue;g_trade_requests++;
      g_execution_verify.attempts++;E2ExecutionResult result;
      string comment=(g_configuration.strategy_mode==E2_STRATEGY_XAU_SESSION_FADE?"E2XAU|":"E2LRB|")+StringSubstr(candidates[i].candidate_id,0,20);
