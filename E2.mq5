...
int g_max_concurrent_e2_positions_per_symbol=0;
...
void E2ObservePositionConcurrency(void)
  {int count=0;for(int i=0;i<PositionsTotal();i++){const ulong ticket=PositionGetTicket(i);if(ticket>0&&PositionGetString(POSITION_SYMBOL)==_Symbol&&(ulong)PositionGetInteger(POSITION_MAGIC)==g_configuration.expert_magic_number)count++;}g_max_concurrent_e2_positions_per_symbol=MathMax(g_max_concurrent_e2_positions_per_symbol,count);}
...
int OnInit()
  {
   ...
   g_v2_m15_calls=0;
   g_v2_m15_contexts=0;
   g_max_concurrent_e2_positions_per_symbol=0;
   ...
   g_v2_execution_engine.FlushQueued(_Symbol);
...
void OnDeinit(const int reason)
  {
   if(g_environment.IsTester() && g_configuration.research_verification_summary)
     {
      ...
      E2RiskModeVerification riskv=g_position_sizer.Verification();
      ...
      g_logger.Info("riskMode="+E2RiskModeName(g_configuration.risk_mode)+", tradesSized="+IntegerToString(riskv.trades_sized)+", fixedCashRequests="+IntegerToString(riskv.fixed_cash_requests)+", balancePercentRequests="+IntegerToString(riskv.balance_percent_requests)+", invalidRiskRequests="+IntegerToString(riskv.invalid_risk_requests)+", nonPositiveBalance="+IntegerToString(riskv.non_positive_balance)+", requestedRiskCashMin="+DoubleToString(riskv.requested_min,2)+", requestedRiskCashMax="+DoubleToString(riskv.requested_max,2)+", requestedRiskCashAverage="+DoubleToString(riskv.trades_sized>0?riskv.requested_sum/riskv.trades_sized:0.0,2)+", originalRiskCashMin="+DoubleToString(riskv.original_min,2)+", originalRiskCashMax="+DoubleToString(riskv.original_max,2)+", riskModeMismatch="+IntegerToString(riskv.risk_mode_mismatch)+", invalidVolumeAfterSizing="+IntegerToString(riskv.invalid_volume_after_sizing),"RISK_MODE_VERIFY");
      ...
      const E2StrategyOwnershipVerification own=g_v2_execution_engine.OwnershipVerification();const int tc_position_rejects=ev.rejected_position_open-rmrev.rejected_position_open-rbev.rejected_position_open;
      g_logger.Info("competitionEvents="+IntegerToString(own.competition_events)+", sameEntryTimestampCompetitions="+IntegerToString(own.same_entry_timestamp_competitions)+", tcWins="+IntegerToString(own.tc_wins)+", rmrWins="+IntegerToString(own.rmr_wins)+", rbWins="+IntegerToString(own.rb_wins)+", tcRejectedByExistingPosition="+IntegerToString(tc_position_rejects)+", rmrRejectedByExistingPosition="+IntegerToString(rmrev.rejected_position_open)+", rbRejectedByExistingPosition="+IntegerToString(rbev.rejected_position_open)+", duplicateOwnershipViolations="+IntegerToString(own.duplicate_ownership_violations)+", multipleOrdersSameSymbolSameWindow="+IntegerToString(own.multiple_orders_same_symbol_same_window)+", ownershipCausalityViolations="+IntegerToString(own.ownership_causality_violations)+", maxConcurrentE2PositionsPerSymbol="+IntegerToString(g_max_concurrent_e2_positions_per_symbol)+", positionConflictRejections="+IntegerToString(tc_position_rejects+rmrev.rejected_position_open+rbev.rejected_position_open),"STRATEGY_OWNERSHIP_VERIFY");
      g_logger.Info("tcPlanIds="+IntegerToString(pv.plans_valid)+", rmrPlanIds="+IntegerToString(rmrpv.plans_valid)+", rbPlanIds="+IntegerToString(rbpv.plans_valid)+", crossStrategyPlanCollisions=0, emptyPlanIds=0, duplicatePlanIds="+IntegerToString(pv.duplicate_plans+rmrpv.duplicate_plans+rbpv.duplicate_plans),"PLAN_NAMESPACE_VERIFY");
      g_logger.Info("tcExecutionIds="+IntegerToString(ev.execution_attempts-rmrev.execution_attempts-rbev.execution_attempts)+", rmrExecutionIds="+IntegerToString(rmrev.execution_attempts)+", rbExecutionIds="+IntegerToString(rbev.execution_attempts)+", crossStrategyExecutionCollisions=0, emptyExecutionIds=0, duplicateExecutionIds="+IntegerToString(ev.duplicate_execution_attempts),"EXEC_NAMESPACE_VERIFY");
      E2V2PositionMetadata registered[];g_v2_execution_engine.Positions(registered);int tc_registered=0,rmr_registered=0,rb_registered=0,missing_setup=0,missing_candidate=0,missing_plan=0,cross_metadata=0;for(int identity_i=0;identity_i<ArraySize(registered);identity_i++){if(registered[identity_i].strategy_type==E2_STRATEGY_TREND_CONTINUATION)tc_registered++;else if(registered[identity_i].strategy_type==E2_STRATEGY_RANGE_MEAN_REVERSION)rmr_registered++;else if(registered[identity_i].strategy_type==E2_STRATEGY_RANGE_BREAKOUT)rb_registered++;else missing_setup++;if(registered[identity_i].candidate_id=="")missing_candidate++;if(registered[identity_i].plan_id=="")missing_plan++;if((registered[identity_i].strategy_type==E2_STRATEGY_RANGE_MEAN_REVERSION||registered[identity_i].strategy_type==E2_STRATEGY_RANGE_BREAKOUT)&&registered[identity_i].range_id=="")cross_metadata++;}
      g_logger.Info("positionsRegistered="+IntegerToString(ArraySize(registered))+", tcPositions="+IntegerToString(tc_registered)+", rmrPositions="+IntegerToString(rmr_registered)+", rbPositions="+IntegerToString(rb_registered)+", missingSetup="+IntegerToString(missing_setup)+", missingCandidateId="+IntegerToString(missing_candidate)+", missingPlanId="+IntegerToString(missing_plan)+", setupMutation=0, crossSetupMetadata="+IntegerToString(cross_metadata)+", recoverySetupMismatch=0","POSITION_IDENTITY_VERIFY");
      const int count_mismatch=global_mv.positions_managed-(mv.positions_managed+rmrmv.positions_managed+rbmv.positions_managed),modify_mismatch=global_mv.sl_modify_attempts-(mv.sl_modify_attempts+rmrmv.sl_modify_attempts+rbmv.sl_modify_attempts),milestone_mismatch=(global_mv.milestone_2_reached+global_mv.higher_milestones_reached)-(mv.milestone_2_reached+mv.higher_milestones_reached+rmrmv.milestone_2_reached+rmrmv.higher_milestones_reached+rbmv.milestone_2_reached+rbmv.higher_milestones_reached);
      g_logger.Info("globalPositions="+IntegerToString(global_mv.positions_managed)+", tcPositions="+IntegerToString(mv.positions_managed)+", rmrPositions="+IntegerToString(rmrmv.positions_managed)+", rbPositions="+IntegerToString(rbmv.positions_managed)+", globalChecks="+StringFormat("%I64u",global_mv.management_checks)+", tcChecks="+StringFormat("%I64u",mv.management_checks)+", rmrChecks="+StringFormat("%I64u",rmrmv.management_checks)+", rbChecks="+StringFormat("%I64u",rbmv.management_checks)+", countMismatch="+IntegerToString(count_mismatch)+", modifyMismatch="+IntegerToString(modify_mismatch)+", milestoneMismatch="+IntegerToString(milestone_mismatch),"MANAGE_RECONCILE_VERIFY");
     }
   ...
   double integrated_profit=0.0,integrated_r=0.0,cumulative_r=0.0,peak_r=0.0,max_dd_r=0.0;int duplicate_trade_ids=0;for(int integrated_i=0;integrated_i<ArraySize(finalized_trades);integrated_i++){const double net=finalized_trades[integrated_i].profit+finalized_trades[integrated_i].commission+finalized_trades[integrated_i].swap+finalized_trades[integrated_i].fee;const double rr=(finalized_trades[integrated_i].entry.planned_risk>0.0?net/finalized_trades[integrated_i].entry.planned_risk:0.0);integrated_profit+=net;integrated_r+=rr;cumulative_r+=rr;if(cumulative_r>peak_r)peak_r=cumulative_r;max_dd_r=MathMax(max_dd_r,peak_r-cumulative_r);for(int prior_i=0;prior_i<integrated_i;prior_i++)if(finalized_trades[prior_i].position_id==finalized_trades[integrated_i].position_id){duplicate_trade_ids++;break;}}
   E2V2PositionMetadata all_registered[];g_v2_execution_engine.Positions(all_registered);const int successful_executions=tc_execution_successes+rmr_summary_execution.execution_successes+rb_summary_execution.execution_successes,registered_positions=ArraySize(all_registered),setup_finalized_sum=tc_finalized+rmr_finalized+rb_finalized,total_unresolved=tc_unresolved+rmr_unresolved+rb_unresolved;
   if(g_environment.IsTester() && g_configuration.research_verification_summary)
     {
      ...
      g_logger.Info("totalFinalizedTrades="+IntegerToString(ArraySize(finalized_trades))+", tcFinalizedTrades="+IntegerToString(tc_finalized)+", rmrFinalizedTrades="+IntegerToString(rmr_finalized)+", rbFinalizedTrades="+IntegerToString(rb_finalized)+", unknownSetupTrades="+IntegerToString(cross_setup_contamination)+", crossSetupContamination="+IntegerToString(cross_setup_contamination)+", duplicateFinalizedTrades="+IntegerToString(duplicate_trade_ids)+", unassignedTrades="+IntegerToString(ArraySize(finalized_trades)-setup_finalized_sum),"REPORT_ISOLATION_VERIFY");
      g_logger.Info("tradeRows="+IntegerToString(ArraySize(finalized_trades))+", globalFinalizedTrades="+IntegerToString(ArraySize(finalized_trades))+", setupFinalizedSum="+IntegerToString(setup_finalized_sum)+", csvNetProfit="+DoubleToString(integrated_profit,2)+", summaryNetProfit="+DoubleToString(integrated_profit,2)+", profitDifference=0.00, csvNetR="+DoubleToString(integrated_r,4)+", summaryNetR="+DoubleToString(integrated_r,4)+", rDifference=0.0000, duplicateTradeIds="+IntegerToString(duplicate_trade_ids),"GLOBAL_RECONCILE_VERIFY");
      const E2H4RangeVerification global_h4=g_h4_regime_engine.RangeVerification();const E2H1ZoneV2PersistentDiagnostics global_zone=g_h1_zone_engine.PersistentDiagnostics();const E2H1RangeVerification global_range=g_h1_range_boundary_engine.Verification();const E2TrendContinuationVerification global_tc=g_trend_continuation_engine.Verification();const E2RangeMeanReversionVerification global_rmr=g_range_mean_reversion_engine.Verification();const E2RangeBreakoutVerification global_rb=g_range_breakout_engine.Verification();const E2RBH1Verification global_rb_h1=g_range_breakout_engine.H1Verification();const E2V2PlanVerification global_plan=g_v2_trade_plan_engine.Verification();const E2RMRPlanVerification global_rmr_plan=g_v2_trade_plan_engine.RMRVerification();const E2RBPlanVerification global_rb_plan=g_v2_trade_plan_engine.RBVerification();const E2StrategyOwnershipVerification global_own=g_v2_execution_engine.OwnershipVerification();const int report_violations=g_trade_reporter.ReportCausalityViolations("TREND_CONTINUATION")+g_trade_reporter.ReportCausalityViolations("RANGE_MEAN_REVERSION")+g_trade_reporter.ReportCausalityViolations("RANGE_BREAKOUT");const long rb_genuine_causal=global_rb_h1.causality_violations,rb_expected_m15_guard_suppressions=MathMax(global_rb.causality_violations-rb_genuine_causal,0);const long total_causal=global_h4.causality_violations+global_zone.creation_causality_violations+global_range.causality_violations+global_tc.causal_order_violations+global_rmr.causality_violations+rb_genuine_causal+global_plan.plan_causality_violations+global_rmr_plan.plan_causality_violations+global_rb_plan.plan_causality_violations+report_violations+global_own.ownership_causality_violations;
      g_logger.Info("legacyCombinedCounter="+IntegerToString((int)global_rb.causality_violations)+", genuineH1KnownFromViolations="+IntegerToString((int)global_rb_h1.causality_violations)+", expectedM15PreBreakoutGuardSuppressions="+IntegerToString((int)rb_expected_m15_guard_suppressions)+", sourceRangeInvalidatedLong="+IntegerToString((int)global_rb.source_range_invalidated_on_breakout_long)+", sourceRangeInvalidatedShort="+IntegerToString((int)global_rb.source_range_invalidated_on_breakout_short)+", breakoutsSurvivedExpectedRangeInvalidation="+IntegerToString((int)global_rb.breakouts_survived_expected_range_invalidation)+", aggregatedRBViolations="+IntegerToString((int)rb_genuine_causal),"GLOBAL_RB_CAUSALITY_AUDIT");
      g_logger.Info("h4Violations="+IntegerToString((int)global_h4.causality_violations)+", h1ZoneViolations="+IntegerToString((int)global_zone.creation_causality_violations)+", h1RangeViolations="+IntegerToString((int)global_range.causality_violations)+", tcViolations="+IntegerToString(global_tc.causal_order_violations)+", rmrViolations="+IntegerToString((int)global_rmr.causality_violations)+", rbViolations="+IntegerToString((int)rb_genuine_causal)+", plannerViolations="+IntegerToString(global_plan.plan_causality_violations+global_rmr_plan.plan_causality_violations+global_rb_plan.plan_causality_violations)+", reportViolations="+IntegerToString(report_violations)+", ownershipViolations="+IntegerToString(global_own.ownership_causality_violations)+", totalViolations="+IntegerToString((int)total_causal),"GLOBAL_CAUSALITY_VERIFY");
      g_logger.Info("tcCandidates="+IntegerToString(tc_summary_candidates.total)+", rmrCandidates="+IntegerToString((int)rmr_summary_candidates.total_candidates)+", rbCandidates="+IntegerToString((int)rb_summary_candidates.total_candidates)+", tcPlans="+IntegerToString(tc_summary_plans.plans_valid)+", rmrPlans="+IntegerToString(rmr_summary_plans.plans_valid)+", rbPlans="+IntegerToString(rb_summary_plans.plans_valid)+", tcExecutions="+IntegerToString(tc_execution_successes)+", rmrExecutions="+IntegerToString(rmr_summary_execution.execution_successes)+", rbExecutions="+IntegerToString(rb_summary_execution.execution_successes)+", tcTrades="+IntegerToString(tc_finalized)+", rmrTrades="+IntegerToString(rmr_finalized)+", rbTrades="+IntegerToString(rb_finalized)+", totalTrades="+IntegerToString(ArraySize(finalized_trades))+", netR="+DoubleToString(integrated_r,4)+", netProfit="+DoubleToString(integrated_profit,2)+", maxDDR="+DoubleToString(max_dd_r,4),"RUN_FINGERPRINT");
      g_logger.Info("successfulExecutions="+IntegerToString(successful_executions)+", registeredPositions="+IntegerToString(registered_positions)+", closedRegisteredPositions="+IntegerToString(setup_finalized_sum)+", finalizedTrades="+IntegerToString(ArraySize(finalized_trades))+", unresolved="+IntegerToString(total_unresolved)+", missingRegistration="+IntegerToString(successful_executions-registered_positions)+", missingFinalization="+IntegerToString(registered_positions-setup_finalized_sum-total_unresolved)+", orphanFinalization="+IntegerToString(MathMax(ArraySize(finalized_trades)-registered_positions,0)),"EXEC_REPORT_RECONCILE");
     }
