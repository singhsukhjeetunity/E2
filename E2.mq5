...
void E2MetadataToReport(const E2OBRPositionMetadata &m,E2ReportEntryData &entry)
  {
   ZeroMemory(entry);entry.symbol=m.symbol;entry.setup_id="OBR";entry.signal_id=m.candidate_id;entry.execution_id=m.execution_id;entry.london_day=m.london_day;entry.direction=m.direction;entry.signal_time=m.breakout_time;entry.signal_known_from=m.breakout_time+PeriodSeconds(PERIOD_M15);entry.request_time=m.entry_time;entry.entry_time=m.entry_time;entry.or_high=m.or_high;entry.or_low=m.or_low;entry.frozen_atr=m.frozen_atr;entry.frozen_adx=m.frozen_adx;entry.requested_entry_price=m.fill_price;entry.fill_price=m.fill_price;entry.structural_stop_price=m.structural_stop;entry.submitted_stop_price=m.submitted_stop;entry.take_profit_price=m.target_price;entry.original_r_price=m.original_r_price;entry.requested_risk_cash=m.requested_risk_cash;entry.actual_risk_cash=m.actual_risk_cash;entry.volume=m.volume;entry.order_ticket=m.order_ticket;entry.entry_deal=m.entry_deal;
  }
...
void E2ExecuteOBRCandidate(const E2OBRCandidate &candidate)
  {
   ...
   E2ExecutionResult result;const string comment="E2OBR|"+candidate.london_day+"|"+(candidate.direction==E2_DIRECTION_LONG?"L":"S");if(!g_order_executor.Execute(request,comment,result)){g_obr_planner.RecordFailure(result.status);return;}
   ...
   if(!g_obr_recovery.Save(metadata))g_obr_planner.RecordRegistrationFailure();E2ReportEntryData entry;E2MetadataToReport(metadata,entry);if(!g_trade_reporter.CaptureEntry(entry))g_obr_planner.RecordRegistrationFailure();g_visualizer.DrawOBRExecution(metadata);
  }
...
int OnInit()
  {
   ...
   g_obr_recovery.Initialize(_Symbol,g_configuration,g_logger);g_obr_planner.Initialize(_Symbol,g_configuration,g_symbol_info,g_position_sizer,g_position_guard,g_obr_recovery,g_logger);g_obr_recovery.DayConsumed(g_obr_engine.LondonDayAtServerTime(TimeCurrent()));
...
void OnDeinit(const int reason)
  {
   ...
   const E2OBRVerification obr_verify=g_obr_engine.Verification();const E2OBRTimeVerification time_verify=g_obr_engine.TimeVerification();g_backtest_summary.OBRVerify(obr_verify);g_backtest_summary.OBRTimeVerify(time_verify);g_backtest_summary.OBRPlanVerify(g_obr_planner.PlanVerification());g_backtest_summary.OBRExecVerify(g_obr_planner.ExecutionVerification());g_backtest_summary.OBRRecoveryVerify(g_obr_recovery.Verification());g_backtest_summary.RiskVerify(g_position_sizer.Verification());
   E2EmitVerification();
