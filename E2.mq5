...
void OnDeinit(const int reason)
  {
   ...
   int tc_finalized=0,rmr_finalized=0,cross_setup_contamination=0;
   for(int report_trade=0;report_trade<ArraySize(finalized_trades);report_trade++){if(finalized_trades[report_trade].entry.strategy_type=="TREND_CONTINUATION")tc_finalized++;else if(finalized_trades[report_trade].entry.strategy_type=="RANGE_MEAN_REVERSION")rmr_finalized++;else cross_setup_contamination++;}
   const int tc_execution_attempts=tc_summary_execution.execution_attempts-rmr_summary_execution.execution_attempts,tc_execution_successes=tc_summary_execution.execution_successes-rmr_summary_execution.execution_successes;
   const int tc_unresolved=g_trade_reporter.UnresolvedForSetup("TREND_CONTINUATION"),rmr_unresolved=g_trade_reporter.UnresolvedForSetup("RANGE_MEAN_REVERSION");
   if(g_environment.IsTester() && g_configuration.research_verification_summary)
     {
      g_logger.Info("candidates="+IntegerToString(tc_summary_candidates.total)+", plannerCandidates="+IntegerToString(tc_summary_plans.candidates_received)+", validPlans="+IntegerToString(tc_summary_plans.plans_valid)+", executionPlans="+IntegerToString(tc_summary_execution.valid_plans_received-rmr_summary_execution.valid_plans_received)+", executionAttempts="+IntegerToString(tc_execution_attempts)+", executionSuccesses="+IntegerToString(tc_execution_successes)+", finalizedTrades="+IntegerToString(tc_finalized)+", unresolved="+IntegerToString(tc_unresolved)+", crossSetupContamination="+IntegerToString(cross_setup_contamination)+", "+g_trade_reporter.InvariantSummary(),"TC_REPORT_VERIFY");
