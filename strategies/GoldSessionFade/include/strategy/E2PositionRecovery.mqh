...
class E2PositionRecovery
  {
private:
   E2Config m_config;ulong m_magic,m_active_position_id;string m_symbol,m_file;bool m_one_per_day,m_is_tester;E2Logger *m_logger;E2BrokerTimeAdapter *m_time;E2RecoveryVerification m_verify;E2DayVerification m_day;E2DayRecoveryDiagnostics m_day_diag;E2RecoveryDiagnostics m_diag;
   string SafeSymbol()const{string s=m_symbol;StringReplace(s,".","_");StringReplace(s,"#","_");return(s);}
   bool RuleDay(const datetime server,int &day)const
     {
      day=0;
...
if(!ReadField(h,f,fields))return(false);m.rule_day=(int)StringToInteger(f);
if(!ReadField(h,f,fields))return(false);m.range_start_rule=(datetime)StringToInteger(f);
if(!ReadField(h,f,fields))return(false);m.range_end_rule=(datetime)StringToInteger(f);
...
if(!ReadField(h,f,fields))return(false);m.extension_distance=StringToDouble(f);
...
public:
   ...
   bool DayConsumed(const datetime server)
   {
      ...
      for(int i=0;i<HistoryDealsTotal();i++)
      {
         ...
         if(!RuleDay(tm,day)){m_day.daily_lock_failure_reason="DEAL_TIME_UNMAPPABLE";return(true);}
      ...
      m_day.daily_lock_failure_reason=(same>0?"NONE":"NO_ENTRY_FOR_RULE_DAY");
   ...
   void AuditDayEntries()
   {
      ...
      for(int i=0;i<HistoryDealsTotal();i++)
      {
         ...
         int day=0;if(!RuleDay(tm,day)){m_day.day_mapping_violations++;continue;}
