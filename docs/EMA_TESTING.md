...
5. Inspect `*_Trades_T.csv`, `*_Signals_S.csv` and `*_Equity_E.csv` in `E2/EMAPullback/` (filenames include the run). Long trades must say LONG. Match run IDs before analyzing.
...
7. Run an identical test twice. Each run must create different filenames in the same strategy folder without overwriting earlier files. Test a symbol with punctuation to verify folder handling.
