...
bool E2ValidateConfiguration(const E2Config &c,string &reason)
  {
   reason="";
   if(c.obr_adx_length<1||c.obr_atr_length<1||!MathIsValidNumber(c.obr_minimum_adx)||c.obr_minimum_adx<0.0||!MathIsValidNumber(c.obr_minimum_range_atr)||c.obr_minimum_range_atr<0.0||!MathIsValidNumber(c.obr_maximum_breakout_gap_atr)||c.obr_maximum_breakout_gap_atr<0.0||!MathIsValidNumber(c.obr_stop_buffer_atr)||c.obr_stop_buffer_atr<0.0||!MathIsValidNumber(c.obr_target_r)||c.obr_target_r<=0.0){reason="OBR indicator/filter/trade values are invalid.";return(false);}
