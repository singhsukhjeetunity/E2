...
struct E2RangeMeanReversionCandidate
  {
   ...
   string candidate_id,symbol,range_id,source_zone_id,lower_zone_id,upper_zone_id;
   ...
   double lower_reference,upper_reference,range_center,range_height,source_zone_lower,source_zone_upper,lower_zone_lower,lower_zone_upper,upper_zone_lower,upper_zone_upper,confirmation_h1_atr;
...
private:
   ...
   void Emit(E2RangeMeanReversionCandidate &output[],const string symbol,const E2H4RegimeResult &h4,const E2H1RangeBoundaryContext &range,const E2RangeMeanReversionClaim &claim,const E2RangeMeanReversionSide &side)
     {
      E2RangeMeanReversionCandidate candidate;ZeroMemory(candidate);candidate.setup_type=E2_STRATEGY_RANGE_MEAN_REVERSION;candidate.symbol=symbol;candidate.range_id=range.range_id;candidate.direction=claim.direction;candidate.source_zone_id=claim.source_zone_id;candidate.lower_zone_id=range.lower_zone_id;candidate.upper_zone_id=range.upper_zone_id;candidate.source_zone_role=(claim.direction==E2_RMR_LONG?E2_H1_ZONE_V2_SUPPORT:E2_H1_ZONE_V2_RESISTANCE);candidate.attempt_number=side.attempt_number;candidate.range_known_from=range.known_from_time;candidate.boundary_visit_time=side.visit_time;candidate.boundary_visit_known_from=side.visit_known_from;candidate.confirmation_candle=claim.confirmation.candle_time;candidate.confirmation_known_from=claim.confirmation.known_from_time;candidate.h4_regime=h4.regime;candidate.h4_regime_known_from=h4.known_from_time;candidate.lower_reference=range.lower_reference_price;candidate.upper_reference=range.upper_reference_price;candidate.range_center=range.range_center;candidate.range_height=range.range_height;candidate.lower_zone_lower=range.lower_zone_lower;candidate.lower_zone_upper=range.lower_zone_upper;candidate.upper_zone_lower=range.upper_zone_lower;candidate.upper_zone_upper=range.upper_zone_upper;candidate.confirmation_h1_atr=range.creation_h1_atr;candidate.source_zone_lower=(claim.direction==E2_RMR_LONG?range.lower_zone_lower:range.upper_zone_lower);candidate.source_zone_upper=(claim.direction==E2_RMR_LONG?range.lower_zone_upper:range.upper_zone_upper);candidate.confirmation=claim.confirmation;candidate.candidate_id=CandidateId(symbol,range,claim.direction,side.attempt_number,candidate.confirmation_known_from);
   ...
   bool Evaluate(const string symbol,const datetime evaluation,const E2H4RegimeResult &h4,const E2H1RangeBoundaryContext &range,const double h1_atr,E2RangeMeanReversionCandidate &output[])
     {
      ...
      if(m_long.state==E2_RMR_BOUNDARY_ACTIVE){E2M15ConfirmationResult confirmation;E2M15ZoneContext zone=Zone(range,E2_RMR_LONG);if(m_confirmation.EvaluateDirectionalRejection(symbol,evaluation,zone,true,confirmation)&&confirmation.passed){m_verify.bullish_rejection_passes++;E2RangeMeanReversionClaim claim;claim.direction=E2_RMR_LONG;claim.confirmation=confirmation;claim.source_zone_id=zone.zone_id;claim.boundary_distance=MathAbs(confirmation.close-range.lower_reference_price);AddClaim(claims,claim);}}
      if(m_short.state==E2_RMR_BOUNDARY_ACTIVE){E2M15ConfirmationResult confirmation;E2M15ZoneContext zone=Zone(range,E2_RMR_SHORT);if(m_confirmation.EvaluateDirectionalRejection(symbol,evaluation,zone,false,confirmation)&&confirmation.passed){m_verify.bearish_rejection_passes++;E2RangeMeanReversionClaim claim;claim.direction=E2_RMR_SHORT;claim.confirmation=confirmation;claim.source_zone_id=zone.zone_id;claim.boundary_distance=MathAbs(confirmation.close-range.upper_reference_price);AddClaim(claims,claim);}}
