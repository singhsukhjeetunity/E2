...
private:
   ...
   void LogPassed(const E2M15ConfirmationResult &result) const
     {
      if(m_logger==NULL || !m_logger.IsDebugEnabled() || !result.passed)return;
   ...
   void LogBoundaryFailure(const E2M15ConfirmationResult &result) const
     {
      if(m_logger==NULL || !m_logger.IsDebugEnabled() || result.passed || !result.history_available || !result.zone_validity_pass || !result.direction_pass || result.zero_range)return;
...
public:
   E2M15ConfirmationEngine(void):m_market(NULL),m_logger(NULL),m_lookback(20),m_body_multiplier(1.25),m_body_range_min(0.60),m_close_fraction(0.20),m_wick_body_min(1.50),m_wick_range_min(0.40) {}
   void Initialize(const E2Config &config,E2MarketData &market,E2Logger &logger)
     {m_market=&market;m_logger=&logger;m_lookback=config.research_m15_body_median_lookback;m_body_multiplier=config.research_m15_momentum_body_multiplier;m_body_range_min=config.research_m15_momentum_body_range_minimum;m_close_fraction=config.research_m15_momentum_closing_location_fraction;m_wick_body_min=config.research_m15_rejection_wick_body_minimum;m_wick_range_min=config.research_m15_rejection_wick_range_minimum;}
   bool Evaluate(const string symbol,const datetime evaluation,const E2M15ZoneContext &zone,E2M15ConfirmationResult &results[])
     {
      ...
      if(m_market==NULL)return(false);MqlRates bars[];if(!m_market.GetClosedBarsAsOf(symbol,PERIOD_M15,evaluation,m_lookback+1,bars))return(false);const int candidate=ArraySize(bars)-1;if(candidate<1)return(false);
      for(int i=0;i<ArraySize(bars);i++)if(!Valid(bars[i])){for(int j=0;j<4;j++)results[j].reason="INVALID_OHLC";return(false);}
      double median=0.0;const bool history=MedianPrecedingBodies(bars,candidate,median);for(int i=0;i<4;i++){results[i].history_available=history;FillMeasurements(results[i],bars[candidate],bars[candidate-1],median);results[i].zone_validity_pass=ZoneValidAt(zone,results[i].known_from_time);}
      EvaluateMomentum(results[0],zone,bars[candidate],bars[candidate-1],true);EvaluateMomentum(results[1],zone,bars[candidate],bars[candidate-1],false);EvaluateRejection(results[2],zone,bars[candidate],true);EvaluateRejection(results[3],zone,bars[candidate],false);
      for(int i=0;i<4;i++){results[i].reason=(results[i].passed ? "PASS" : (!history ? "INSUFFICIENT_HISTORY" : (results[i].zero_range ? "ZERO_RANGE" : "CONDITIONS_FAILED")));LogPassed(results[i]);LogBoundaryFailure(results[i]);}return(true);
     }
  };
