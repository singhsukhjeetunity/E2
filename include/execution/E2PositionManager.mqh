...
class E2PositionManager
  { private: ulong m_magic; E2Logger *m_logger; E2ManagedPosition m_positions[];
    ...
    bool Refresh(){E2ManagedPosition next[];for(int i=0;i<PositionsTotal();i++){ulong t=PositionGetTicket(i);if(t>0&&(ulong)PositionGetInteger(POSITION_MAGIC)==m_magic){int n=ArraySize(next);ArrayResize(next,n+1);next[n]=Read();}}
      for(int i=0;i<ArraySize(m_positions);i++){bool present=false;for(int j=0;j<ArraySize(next);j++)if(next[j].ticket==m_positions[i].ticket){present=true;break;}if(!present&&m_logger!=NULL)m_logger.Info("Position closed/disappeared: ticket="+StringFormat("%I64u",m_positions[i].ticket)+".","PositionManager");}
      for(int i=0;i<ArraySize(next);i++){int previous=Find(next[i].ticket);if(previous<0&&m_logger!=NULL)m_logger.Info("Recovered position: symbol="+next[i].symbol+", ticket="+StringFormat("%I64u",next[i].ticket)+", volume="+DoubleToString(next[i].volume,4)+".","PositionManager");else if(previous>=0&&(next[i].volume!=m_positions[previous].volume||next[i].stop_loss!=m_positions[previous].stop_loss||next[i].take_profit!=m_positions[previous].take_profit)&&m_logger!=NULL)m_logger.Info("Position modified: ticket="+StringFormat("%I64u",next[i].ticket)+".","PositionManager");}
