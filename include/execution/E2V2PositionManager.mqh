...
struct E2V2ManagedState
  {ulong position_id,ticket;string symbol;E2TradeDirection direction;double entry,original_sl,original_r;E2V2ManagementBranch branch;int maximum_milestone;bool counted;};

class E2V2PositionManager
  {
private:
   ulong m_magic;datetime m_initialized_at;E2Logger *m_logger;CTrade m_trade;E2V2ManagedState m_states[];E2V2ManagementDiagnostics m_diagnostics;
   ...
   void DeleteState(const ulong position_id)const{GlobalVariableDel(Key(position_id,"ENTRY"));GlobalVariableDel(Key(position_id,"ORIGSL"));GlobalVariableDel(Key(position_id,"R"));GlobalVariableDel(Key(position_id,"BRANCH"));GlobalVariableDel(Key(position_id,"MILESTONE"));}
   ...
   void ManageSelected(E2V2ManagedState &state)
     {
      m_diagnostics.management_checks++;MqlTick tick;if(!SymbolInfoTick(state.symbol,tick)||tick.bid<=0.0||tick.ask<tick.bid)return;const double favorable=(state.direction==E2_DIRECTION_BUY?tick.bid-state.entry:state.entry-tick.ask)/state.original_r;const int observed=(int)MathFloor(favorable+1e-10);
      if(observed>state.maximum_milestone){if(state.maximum_milestone<2&&observed>=2)m_diagnostics.milestone_2_reached++;const int higher_start=MathMax(state.maximum_milestone+1,3);if(observed>=higher_start)m_diagnostics.higher_milestones_reached+=observed-higher_start+1;state.maximum_milestone=observed;Set(state.position_id,"MILESTONE",(double)observed);}if(state.maximum_milestone<2)return;
      E2SymbolSpecification spec;spec.symbol=state.symbol;spec.digits=(int)SymbolInfoInteger(state.symbol,SYMBOL_DIGITS);spec.point=SymbolInfoDouble(state.symbol,SYMBOL_POINT);spec.tick_size=SymbolInfoDouble(state.symbol,SYMBOL_TRADE_TICK_SIZE);if(spec.point<=0.0||spec.tick_size<=0.0){m_diagnostics.broker_constraint_deferrals++;return;}const int locked_r=state.maximum_milestone-1;const double theoretical=(state.direction==E2_DIRECTION_BUY?state.entry+locked_r*state.original_r:state.entry-locked_r*state.original_r);const double desired=(state.direction==E2_DIRECTION_BUY?FloorPrice(theoretical,spec):CeilPrice(theoretical,spec));const double existing=PositionGetDouble(POSITION_SL);
      if((state.direction==E2_DIRECTION_BUY&&desired<=existing+spec.tick_size*0.1)||(state.direction==E2_DIRECTION_SELL&&desired>=existing-spec.tick_size*0.1)){const double existing_locked=(state.direction==E2_DIRECTION_BUY?existing-state.entry:state.entry-existing)/state.original_r;if(existing_locked>locked_r+0.25)m_diagnostics.stop_regression_violations++;else m_diagnostics.duplicate_modify_suppressed++;return;}
      const double market=(state.direction==E2_DIRECTION_BUY?tick.bid:tick.ask);const double minimum=MathMax((double)SymbolInfoInteger(state.symbol,SYMBOL_TRADE_STOPS_LEVEL),(double)SymbolInfoInteger(state.symbol,SYMBOL_TRADE_FREEZE_LEVEL))*spec.point;if((state.direction==E2_DIRECTION_BUY&&desired>market-minimum+1e-12)||(state.direction==E2_DIRECTION_SELL&&desired<market+minimum-1e-12)){m_diagnostics.broker_constraint_deferrals++;return;}
      const double tp=PositionGetDouble(POSITION_TP);m_diagnostics.sl_modify_attempts++;m_trade.SetExpertMagicNumber(m_magic);const bool modified=m_trade.PositionModify(state.ticket,desired,tp);const uint retcode=m_trade.ResultRetcode();if(!modified||(retcode!=TRADE_RETCODE_DONE&&retcode!=TRADE_RETCODE_NO_CHANGES)){m_diagnostics.sl_modify_failures++;return;}if(retcode==TRADE_RETCODE_NO_CHANGES){m_diagnostics.duplicate_modify_suppressed++;return;}m_diagnostics.sl_modify_success++;if(m_logger!=NULL)m_logger.Info("ticket="+StringFormat("%I64u",state.ticket)+", direction="+(state.direction==E2_DIRECTION_BUY?"LONG":"SHORT")+", entry="+DoubleToString(state.entry,spec.digits)+", originalSL="+DoubleToString(state.original_sl,spec.digits)+", R="+DoubleToString(state.original_r,spec.digits)+", milestone="+IntegerToString(state.maximum_milestone)+", lockedR="+IntegerToString(locked_r)+", desiredSL="+DoubleToString(theoretical,spec.digits)+", appliedSL="+DoubleToString(desired,spec.digits)+".","TCV2_MANAGE");
     }
...
public:
   E2V2PositionManager(void):m_magic(0),m_initialized_at(0),m_logger(NULL){ZeroMemory(m_diagnostics);}
   void Initialize(const E2Config &config,E2Logger &logger){m_magic=config.expert_magic_number;m_initialized_at=TimeCurrent();m_logger=&logger;m_trade.SetAsyncMode(false);m_trade.SetExpertMagicNumber(m_magic);ArrayResize(m_states,0);ZeroMemory(m_diagnostics);CleanupOrphanedGlobals();Check();}
