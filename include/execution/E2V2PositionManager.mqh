...
class E2V2PositionManager
  {
private:
   ulong m_magic;datetime m_initialized_at;E2Logger *m_logger;CTrade m_trade;E2V2ManagedState m_states[];E2V2ManagementDiagnostics m_diagnostics;E2RMRManagementDiagnostics m_rmr_diagnostics,m_rb_diagnostics;
   ...
   void ObserveSelected()
     {
      if(!OwnedPositionSelected())return;const ulong id=(ulong)PositionGetInteger(POSITION_IDENTIFIER);int at=Find(id);if(at<0){E2V2ManagedState state;ZeroMemory(state);bool recovered=false;if(!LoadSelected(state,recovered)){m_diagnostics.invalid_original_r++;if(state.strategy_type==E2_STRATEGY_RANGE_MEAN_REVERSION)m_rmr_diagnostics.invalid_original_r++;else if(state.strategy_type==E2_STRATEGY_RANGE_BREAKOUT)m_rb_diagnostics.invalid_original_r++;return;}at=ArraySize(m_states);ArrayResize(m_states,at+1);m_states[at]=state;if(recovered){m_diagnostics.recovered_positions++;if(state.strategy_type==E2_STRATEGY_RANGE_MEAN_REVERSION)m_rmr_diagnostics.recovered_positions++;else if(state.strategy_type==E2_STRATEGY_RANGE_BREAKOUT)m_rb_diagnostics.recovered_positions++;}}
      m_states[at].ticket=(ulong)PositionGetInteger(POSITION_TICKET);if(!m_states[at].counted){m_states[at].counted=true;m_diagnostics.positions_managed++;if(m_states[at].strategy_type==E2_STRATEGY_RANGE_MEAN_REVERSION)m_rmr_diagnostics.positions_managed++;else if(m_states[at].strategy_type==E2_STRATEGY_RANGE_BREAKOUT)m_rb_diagnostics.positions_managed++;if(m_states[at].branch==E2_V2_MANAGEMENT_FIXED_2R)m_diagnostics.fixed_2r_positions_observed++;else if(m_states[at].branch==E2_V2_MANAGEMENT_ZONE_TARGET_TRAILING)m_diagnostics.trailing_positions_managed++;}
   ...
   E2V2ManagementDiagnostics TCDiagnostics()const
     {
      ...
      value.positions_managed-=m_rmr_diagnostics.positions_managed+m_rb_diagnostics.positions_managed;
      value.trailing_positions_managed-=m_rmr_diagnostics.positions_managed+m_rb_diagnostics.positions_managed;
      value.management_checks-=m_rmr_diagnostics.management_checks+m_rb_diagnostics.management_checks;
      value.milestone_2_reached-=m_rmr_diagnostics.milestone_2_reached+m_rb_diagnostics.milestone_2_reached;
      value.higher_milestones_reached-=m_rmr_diagnostics.higher_milestones_reached+m_rb_diagnostics.higher_milestones_reached;
      value.sl_modify_attempts-=m_rmr_diagnostics.sl_modify_attempts+m_rb_diagnostics.sl_modify_attempts;
      value.sl_modify_success-=m_rmr_diagnostics.sl_modify_success+m_rb_diagnostics.sl_modify_success;
      value.sl_modify_failures-=m_rmr_diagnostics.sl_modify_failures+m_rb_diagnostics.sl_modify_failures;
      value.broker_constraint_deferrals-=m_rmr_diagnostics.broker_constraint_deferrals+m_rb_diagnostics.broker_constraint_deferrals;
      value.stop_regression_violations-=m_rmr_diagnostics.stop_regression_violations+m_rb_diagnostics.stop_regression_violations;
      value.invalid_original_r-=m_rmr_diagnostics.invalid_original_r+m_rb_diagnostics.invalid_original_r;
      value.recovered_positions-=m_rmr_diagnostics.recovered_positions+m_rb_diagnostics.recovered_positions;
      return(value);
     }
   E2RMRManagementDiagnostics RMRDiagnostics()const{return(m_rmr_diagnostics);}
   E2RMRManagementDiagnostics RBDiagnostics()const{return(m_rb_diagnostics);}
  };
