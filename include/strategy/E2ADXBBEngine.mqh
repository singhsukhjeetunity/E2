   ...
   void ProcessBar(const MqlRates &bar,E2ADXBBCandidate &candidates[])
     {
      ...
      if(m_config.adxbb_require_bb_reentry_confirmation){long_signal=false;short_signal=false;if(m_pending_setup_direction!=E2_DIRECTION_NONE){const bool immediate=(bar.time==m_pending_setup_time+PeriodSeconds(PERIOD_M5));const bool confirmed=(immediate&&bb_valid&&atr_valid&&((m_pending_setup_direction==E2_DIRECTION_LONG&&bar.close>=lower)||(m_pending_setup_direction==E2_DIRECTION_SHORT&&bar.close<=upper)));if(confirmed){long_signal=(m_pending_setup_direction==E2_DIRECTION_LONG);short_signal=(m_pending_setup_direction==E2_DIRECTION_SHORT);DebugReentry("ADXBB_REENTRY_CONFIRM",m_pending_setup_direction,"CONFIRMED",bar,adx,lower,upper);}else DebugReentry("ADXBB_REENTRY_CONFIRM",m_pending_setup_direction,"EXPIRED",bar,adx,lower,upper);}m_pending_setup_direction=(long_setup?E2_DIRECTION_LONG:(short_setup?E2_DIRECTION_SHORT:E2_DIRECTION_NONE));m_pending_setup_time=(m_pending_setup_direction==E2_DIRECTION_NONE?0:bar.time);if(m_pending_setup_direction!=E2_DIRECTION_NONE)DebugReentry("ADXBB_REENTRY_SETUP",m_pending_setup_direction,"PENDING",bar,adx,lower,upper);}
      ...
      const E2TradeDirection signal_direction=(long_signal?E2_DIRECTION_LONG:(short_signal?E2_DIRECTION_SHORT:E2_DIRECTION_NONE));if(signal_direction==E2_DIRECTION_NONE)return;const E2TradeDirection direction=(!m_config.adxbb_invert_trade_direction?signal_direction:(signal_direction==E2_DIRECTION_LONG?E2_DIRECTION_SHORT:E2_DIRECTION_LONG));DebugDirection(signal_direction,direction);
...
public:
   ...
   void Shutdown(void){m_validation_csv.Close();}
  };
