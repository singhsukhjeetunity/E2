...
public:
   E2ADXBBEngine(void):m_market(NULL),m_logger(NULL),m_symbol(""),m_last_processed_bar(0),m_pending_setup_time(0),m_pending_setup_direction(E2_DIRECTION_NONE),m_pending_hybrid_decision(E2_HYBRID_SKIP),m_have_previous(false),m_previous_high(0.0),m_previous_low(0.0),m_previous_close(0.0),m_bb_buffer_price(0.0){ZeroMemory(m_signal_verify);ZeroMemory(m_indicator_verify);ResetRma(m_tr_di);ResetRma(m_plus_dm);ResetRma(m_minus_dm);ResetRma(m_dx);ResetRma(m_atr);}
   bool Initialize(const string symbol,const E2Config &config,const double pip_size,const string run_id,const string config_hash,E2MarketData &market,E2Logger &logger)
     {
