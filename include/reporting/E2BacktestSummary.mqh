   ...
   bool Initialize(const bool csv_enabled,const E2Config &config,const string symbol,const string run_id,E2Logger &logger)
     {
      ...
      string header[]={"setup_type","candidates","entry_windows_reached","valid_plans","execution_attempts","executed_trades","closed_finalized_trades","fixed_2r_trades","fixed_2r_net_r","fixed_2r_net_profit","zone_trailing_trades","zone_trailing_net_r","zone_trailing_net_profit","symbol","run_id","test_start","test_end","trades","wins","losses","breakeven","win_rate_pct","gross_profit","gross_loss","net_profit","net_r","average_r","median_r","average_win_r","average_loss_r","best_trade_r","worst_trade_r","profit_factor","expectancy_r","max_consecutive_wins","max_consecutive_losses","max_drawdown_r","max_drawdown_r_percent","average_holding_minutes","median_holding_minutes","long_trades","short_trades","long_net_r","short_net_r","open_or_unresolved","tp_count","sl_count","other_exit_count","london_trades","london_net_r","london_win_rate_pct","new_york_trades","new_york_net_r","new_york_win_rate_pct","overlap_trades","overlap_net_r","average_entry_adx","median_entry_adx","initial_deposit","final_balance","final_equity","absolute_net_change","return_percent","finalized_trade_net_profit","account_net_change","reconciliation_difference","trend_timeframe","zone_timeframe","confirmation_timeframe","swing_sensitivity","trend_structure_lookback_bars","adx_enabled","adx_period","adx_minimum_threshold","zone_lookback_bars","risk_percent","london_session_enabled","new_york_session_enabled","london_session_hours","new_york_session_hours","broker_utc_offset_hours","news_filter_enabled","news_buffer_before_minutes","news_buffer_after_minutes","news_high_impact_only","max_spread_pips","expert_magic_number"};
      return(m_csv.WriteHeader(header));
     }
   void FinalizeSetup(const bool tester,const string setup_type,E2ReportedTrade &all_trades[],const int unresolved,const int candidates,const int entry_windows_reached,const int valid_plans,const int execution_attempts,const int executed_trades)
     {
      if(!tester || !m_initialized) return;
      const double epsilon=0.00000001;
      E2ReportedTrade trades[];ArrayResize(trades,0);for(int source=0;source<ArraySize(all_trades);source++){if(all_trades[source].entry.strategy_type!=setup_type)continue;const int at=ArraySize(trades);ArrayResize(trades,at+1);trades[at]=all_trades[source];}
      SortChronologically(trades);
      const int count=ArraySize(trades);
      int wins=0,losses=0,breakeven=0,longs=0,shorts=0,tp=0,sl=0,other=0,london=0,newyork=0,overlap=0,fixed_2r=0,zone_trailing=0;
      int consecutive_wins=0,consecutive_losses=0,max_wins=0,max_losses=0,london_wins=0,newyork_wins=0;
      double gross_profit=0.0,gross_loss=0.0,net_profit=0.0,net_r=0.0,long_r=0.0,short_r=0.0,london_r=0.0,newyork_r=0.0,overlap_r=0.0,win_r=0.0,loss_r=0.0,best_r=0.0,worst_r=0.0,fixed_2r_r=0.0,fixed_2r_profit=0.0,zone_trailing_r=0.0,zone_trailing_profit=0.0;
      ...
      for(int i=0;i<count;i++)
        {
         ...
         net_profit+=net; net_r+=r;if(trades[i].entry.management_branch=="FIXED_2R"){fixed_2r++;fixed_2r_r+=r;fixed_2r_profit+=net;}else if(trades[i].entry.management_branch=="ZONE_TARGET_TRAILING"){zone_trailing++;zone_trailing_r+=r;zone_trailing_profit+=net;}
      ...
      if(m_logger!=NULL) m_logger.Info("Setup="+setup_type+", Candidates="+IntegerToString(candidates)+", EntryWindows="+IntegerToString(entry_windows_reached)+", ValidPlans="+IntegerToString(valid_plans)+", ExecutionAttempts="+IntegerToString(execution_attempts)+", ExecutedTrades="+IntegerToString(executed_trades)+", ClosedTrades="+IntegerToString(count)+", Trades="+IntegerToString(count)+", Wins="+IntegerToString(wins)+", Losses="+IntegerToString(losses)+", BE="+IntegerToString(breakeven)+", WinRate="+F(count>0 ? wins*100.0/count : 0.0,2)+"%, NetR="+F(net_r,2)+", AvgR="+(count>0 ? F(net_r/count,3) : "NA")+", PF="+pf+", MaxDDR="+F(max_dd_r,2)+", NetProfit="+F(net_profit,2)+", OpenOrUnresolved="+IntegerToString(unresolved)+".","RESULT");
     }
