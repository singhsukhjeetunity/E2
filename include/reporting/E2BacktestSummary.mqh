...
public:
   ...
   void Finalize(const bool tester,E2ReportedTrade &all_trades[],const int unresolved,const int candidates,const int entry_windows_reached,const int valid_plans,const int execution_attempts,const int executed_trades)
     {
      if(!tester || !m_initialized) return;
      const double epsilon=0.00000001;
      E2ReportedTrade trades[];ArrayResize(trades,0);for(int source=0;source<ArraySize(all_trades);source++){if(all_trades[source].entry.strategy_type!="TREND_CONTINUATION")continue;const int at=ArraySize(trades);ArrayResize(trades,at+1);trades[at]=all_trades[source];}
