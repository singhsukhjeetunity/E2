...
- M15 08:00–09:00 Europe/London and 09:30–10:30 America/New_York opening-range construction, ATR/ADX filters, breakout checks and deterministic candidate generation are implemented.
...
- One successful fill per symbol/selected-session local day is enforced across restart; failures do not consume the day. Session configuration must remain unchanged for the lifecycle of an open E2 position.
...
`[E2_INPUT_VERIFY]` must report 38 exposed inputs with zero dead inputs, duplicates and invalid mappings.

`[OBR_VERIFY]`, `[OBR_TIME_VERIFY]`, `[OBR_SESSION_VERIFY]`, `[OBR_WEEKDAY_VERIFY]`, `[OBR_PLAN_VERIFY]`, `[OBR_EXEC_VERIFY]`, `[OBR_RECOVERY_VERIFY]` and `[E2_RISK_VERIFY]` report signal, session mapping, planning, execution, persistence and risk diagnostics.
